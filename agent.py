...
def evaluate_market_with_gemini(
  ...
  m1_prompt_block = (
      f'- M1 Candles (Ultra-Micro Structuur & Volume Spikes - Laatste 20):'
      f' {json.dumps(candles_1m[-20:])}\n'
      if candles_1m
      else '- M1 Candles: NIET ACTIEF (Scalp Mode is UIT)\n'
  )

  prompt = (
      'SYSTEM INSTRUCTIONS: QUANTITATIVE CRYPTO TRADING CO-PILOT ('
      + str(symbol)
      + ')\n'
      '1. ROL & HOOFDDOEL: Je bent een meedogenloze, kwantitatieve Trading'
      ' Analyst Co-Pilot gespecialiseerd in Crypto. Je adviseert op basis van'
      ' pure statistische Expected Value (+EV), Adjusted Expected Value'
      ' (EV_adj = T * EV), waarschijnlijkheidsverdelingen, Order Fill Chance (T)'
      ' en strikt risicobeheer. Er is geen ruimte voor emotionele ruis of vage'
      ' voorspellingen.\n\n'
      'CONTEXT BTCUSDT: Price = $'
      + str(btc_context['close'])
      + ', 15m Trend = '
      + str(btc_context['trend'])
      + '\n'
      'HARD KEY LEVELS VOOR '
      + str(symbol)
      + ': '
      + json.dumps(calculated_levels)
      + '\n\n'
      'VERPLICHTE BEREKENDE HARD TP1 TARGETS:\n'
      '- Als LONG trade: TP1 IS VERPLICHT MATEMATISCH $ '
      + str(nearest_long_tp)
      + '\n'
      '- Als SHORT trade: TP1 IS VERPLICHT MATEMATISCH $ '
      + str(nearest_short_tp)
      + '\n\n'
      'TARGET ASSET HISTORIE DATA ('
      + str(symbol)
      + '):\n'
      '- 1D Candles (Laatste 10): '
      + json.dumps(candles_1d[-10:])
      + '\n'
      '- 4H Candles (Laatste 15): '
      + json.dumps(candles_4h[-15:])
      + '\n'
      '- 1H Candles (Laatste 20): '
      + json.dumps(candles_1h[-20:])
      + '\n'
      '- 15m Candles (Laatste 20): '
      + json.dumps(candles_15m[-20:])
      + '\n'
      '- M5 Candles (Laatste 20): '
      + json.dumps(candles_5m[-20:])
      + '\n'
      '- M3 Candles (Micro Reversal & Volume - Laatste 20): '
      + json.dumps(candles_3m[-20:])
      + '\n'
      + m1_prompt_block
      + '\nPLAYBOOK SPECIFIEKE SL / TP EXECUTION REGELS:\n'
      '1. ALS PLAYBOOK = [SCALP RECLAIM] (M3/M5 Micro Reclaim):\n'
      '   - Stop Loss (SL): Strak onder/boven M3/M5 wick (Minimaal '
      + str(min_sl_pct)
      + '%).\n'
      '   - TP1 Level (70% SCALE-OUT): Eerstvolgende M15 of 1H Micro-level.\n'
      '   - R:R Target: TP1 vanaf 1.2R tot 2.0R is voldoende voor een GO.\n'
      '   - ENTRY TRIGGER: Voor Scalp Reclaims is een 15m close OPTIONEEL. Een'
      ' M3 Close met een bevestigde M1/M3 Reversal Candle (volume >= 1.5x SMA 9)'
      ' is VOLDOENDE voor een GO!\n'
      ...
      '   - TP1 Level (30% SCALE-OUT): Major Daily/Weekly'
      ' Resistance/Support.\n'
      ...
      '4. Als R:R naar TP1 voor Optie A of B < 1.20R IS HET VERDICT AUTOMATISCH'
      " 'NO-GO'!\n"
      '5. MINIMUM SL AFSTAND: De afstand tussen Entry en SL MOET minimaal '
      + str(min_sl_pct)
      + '% bedragen op deze asset.\n'
      '6. Formule EV_adj = T * ((P * R_gewogen) - ((1 - P) * 1R)). Reken dit'
      ' MATHEMATISCH EXACT UIT!\n'
      '7. GEEN BLINDE LIMIT ORDERS: Optie D mag alleen gekozen worden als er al'
      ' een M3/M5 reversal candle IS AFGEROND!\n\n'
      'KWANTITATIEVE SCORING MATRIX:\n'
      '1. Trend (35%) | 2. Level Kwaliteit (30%) | 3. Displacement & Micro'
      ' (20%) | 4. Session Timing (15%)\n'
      'Rating: A+ (>=85%), A (65-84%), B (<65% -> AUTOMATISCH NO-GO)\n\n'
      '3-TRAPS VERDICT REGELS:\n'
      '1. PRE-TRADE ALERT: Prijs/wick binnen <= 2.0% van KEY LEVEL, maar nog'
      ' geen reversal.\n'
      '2. WATCHLIST: Key Level geraakt, 15m close (Day Sweep/Swing) of M3'
      ' Opbouw (Scalp) gevalideerd, maar M3/M5 reversal nog in aanbouw.\n'
      '3. GO: (a) Voor Day Sweep/Swing: 15m Full Body Close + M3/M5 Reversal'
      ' BEVESTIGD. (b) Voor Scalp Reclaim: M3 Close + M1/M3 Reversal BEVESTIGD.'
      ' Beiden eisen Score >= 65%, EV_adj > +0.30R en R:R naar TP1 >= 1.2R.\n'
      '4. NO-GO: Score < 65%, onvoldoende R:R (<1.2R) naar TP1, of SL < '
      + str(min_sl_pct)
      + '%.\n\n'
      "OUTPUT FORMAT BIJ 'NO-GO':\n"
      '**GO / NO-GO VERDICT:** **[NO-GO]** *(Rating: B | Score: X% | EV_adj:'
      ' -X.XX R)*\n'
      'Korte Analyse: (Leg uit waarom de R:R onvoldoende is naar TP1, de SL te'
      ' krap is (<'
      + str(min_sl_pct)
      + '%), of de M3/M5 reversal ontbreekt).\n\n'
      "OUTPUT FORMAT BIJ 'PRE-TRADE ALERT':\n"
      '**PRE-TRADE ALERT (KLAARZITTEN)** - '
      + str(symbol)
      + '\n'
      '- **Afstand tot S/R Level:** ~X.XX% (Actuele koers: $'
      + str(current_live_candle['close'])
      + ' vs Key Level: $XX.XX)\n'
      ...
      '**GO / NO-GO VERDICT:** **[GO | WATCHLIST]** *(Rating: [A+ | A] | Score:'
      ' X% | MAX EV_adj: +X.XX R)*\n\n'
      '**ALPHA TRADE ANALYSIS ('
      + str(symbol)
      + '):**\n'
      '- **BTC Context:** BTC Price = $'
      + str(btc_context['close'])
      + ' ('
      + str(btc_context['trend'])
      + ')\n'
      '- **Relative Strength/Weakness:** [Beschrijf of '
      + str(symbol)
      + ' zwakker/sterker is dan BTC en waarom dit extra EV geeft].\n\n'
      '**EXECUTION SUMMARY ('
      + str(symbol)
      + ' - [Long / Short]):**\n'
      '- **Playbook Type & Profile:** [Swing Breakout | Day Sweep | Scalp'
      ' Reclaim]\n'
      '- **Huidige Prijs:** $'
      + str(current_live_candle['close'])
      + '\n'
      ...
      '- **TP1 Level:** **$XX.XX** *(Scale-out: 70% Scalp | 50% Day Sweep | 30%'
      ' Swing)*\n'
      '- **TP2 Level:** **$XX.XX**\n'
      '- **Runner:** **$XX.XX**\n'
      '- **Max Adjusted EV (EV_adj):** **+X.XX R**\n\n'
      '### Execution Optimization Matrix\n'
      '| Parameter | Option A (Cons.) | Option B (Sweet Spot) | Option C (Aggr.'
      ' SL) | Option D (Retest Reversal - MAX EV_adj) |\n'
      '| :--- | :--- | :--- | :--- | :--- |\n'
      ...
      '**Korte Analyse:** (Max 2 zinnen met exacte reden, Daily/4H/1H niveau,'
      ' BTC-correlatie en eventuele Relative Strength/Weakness Bonus).'
  )

  models_to_try = ['gemini-1.5-pro-002', 'gemini-1.5-flash-002']
  ...
  for model_name in models_to_try:
    try:
      response = ai_client.models.generate_content(
          model=model_name,
          contents=prompt,
          # Dwing pure tekstgeneratie af om SDK Automatic Function Calling (AFC) ruis te omzeilen
          config={'tools': []},
      )
...
def run_scanner():
  ...
  for symbol in SYMBOLS:
    try:
      ...
      # 🐞 CONSOLE DEBUG PRINT: Print altijd de eerste regel van Vertex AI op Render!
      if analysis:
