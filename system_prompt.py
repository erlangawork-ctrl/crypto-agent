...
# Minimale HTTP Server om Render tevreden te stellen
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b"Crypto Agent is Running 24/7!")

def start_health_check_server():
    port = int(os.environ.get("PORT", 10000))
    server = HTTPServer(('0.0.0.0', port), SimpleHTTPRequestHandler)
    print(f"Health check server gestart op poort {port}")
...
if __name__ == "__main__":
    # Start de health check server in een achtergrond-thread
